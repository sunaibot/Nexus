...
// 更新插件显示配置
export async function updatePluginDisplayConfig(
  pluginId: string,
  config: Partial<Omit<PluginDisplayConfig, 'id' | 'pluginId' | 'createdAt' | 'updatedAt'>>
): Promise<PluginDisplayConfig> {
  ...
  if (!res.ok) {
    ...
    throw new Error(error.message || '更新插件显示配置失败')
  }
...
// 批量更新插件显示配置
export async function batchUpdatePluginDisplayConfigs(
  configs: Array<{ pluginId: string } & Partial<Omit<PluginDisplayConfig, 'id' | 'createdAt' | 'updatedAt'>>>
): Promise<PluginDisplayConfig[]> {
  const token = localStorage.getItem('token')
  const res = await fetch(`${API_BASE}/api/v2/plugin-display-configs/batch/update`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': token ? `Bearer ${token}` : '',
  ...
  if (!res.ok) {
    const error = await res.json()
    throw new Error(error.message || '批量更新插件显示配置失败')
  }
...
// 删除插件显示配置
export async function deletePluginDisplayConfig(pluginId: string): Promise<{ message: string }> {
  const token = localStorage.getItem('token')
  const res = await fetch(`${API_BASE}/api/v2/plugin-display-configs/${pluginId}`, {
    method: 'DELETE',
    headers: {
  ...
  if (!res.ok) {
    const error = await res.json()
    throw new Error(error.message || '删除插件显示配置失败')
  }
...
// 重置插件显示配置为默认值
export async function resetPluginDisplayConfig(pluginId: string): Promise<PluginDisplayConfig> {
  const token = localStorage.getItem('token')
  const res = await fetch(`${API_BASE}/api/v2/plugin-display-configs/${pluginId}/reset`, {
    method: 'POST',
    headers: {
      'Authorization': token ? `Bearer ${token}` : '',
    },
  ...
  if (!res.ok) {
    const error = await res.json()
    throw new Error(error.message || '重置插件显示配置失败')
  }
